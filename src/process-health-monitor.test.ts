...
describe('isComplexTask', () => {
  it('should identify database tasks as complex', () => {
    expect(isComplexTask('Add database migration')).toBe(true);
    expect(isComplexTask('Fix database connection pool')).toBe(true);
  });

  it('should identify API tasks as complex', () => {
    expect(isComplexTask('Create API endpoint')).toBe(true);
    expect(isComplexTask('Integrate with external API')).toBe(true);
  });

  it('should identify integration tasks as complex', () => {
    expect(isComplexTask('Supabase integration')).toBe(true);
    expect(isComplexTask('Backend refactor')).toBe(true);
  });

  it('should identify simple tasks as non-complex', () => {
    expect(isComplexTask('Fix UI bug in button')).toBe(false);
    expect(isComplexTask('Update README')).toBe(false);
    expect(isComplexTask('Change color of header')).toBe(false);
  });

  it('should be case insensitive', () => {
    expect(isComplexTask('DATABASE migration')).toBe(true);
    expect(isComplexTask('Api Endpoint')).toBe(true);
  });
});
...
describe('ProcessHealthMonitor', () => {
  ...
  describe('constructor', () => {
    it('should create monitor with default config', () => {
      const defaultMonitor = new ProcessHealthMonitor();
    ...
    it('should merge custom config with defaults', () => {
      const customMonitor = new ProcessHealthMonitor({
  ...
  describe('getProcessMetrics', () => {
    it('should return metrics for a running process', async () => {
      ...
      mockExec.mockImplementation((_cmd: string, _options: unknown, callback: (error: null, result: { stdout: string; stderr: string }) => void) => {
    ...
    it('should handle exec errors gracefully', async () => {
      const mockExec = require('child_process').exec as jest.Mock;
      mockExec.mockImplementation((_cmd: string, _options: unknown, callback: (error: Error, result: { stdout: string; stderr: string }) => void) => {
  ...
  describe('checkProcessHealth', () => {
    ...
    it('should detect stuck process (complex task with shorter timeout)', async () => {
      const mockExec = require('child_process').exec as jest.Mock;
      const complexMonitor = new ProcessHealthMonitor({
    ...
    it('should warn on high CPU usage', async () => {
      const mockExec = require('child_process').exec as jest.Mock;
      const highCpuMonitor = new ProcessHealthMonitor({
        maxCpuPercent: 80,
      });
    ...
    it('should warn on high memory usage', async () => {
      const mockExec = require('child_process').exec as jest.Mock;
      const lowMemoryMonitor = new ProcessHealthMonitor({
        maxMemoryMB: 100,
      });
  ...
  describe('runHealthChecks', () => {
    ...
    it('should kill stuck processes and call callback', async () => {
      ...
      const stuckMonitor = new ProcessHealthMonitor({
        normalTimeoutMs: -1,
        complexTimeoutMs: -1,
      });
...
describe('createProcessHealthMonitor', () => {
  it('should create a ProcessHealthMonitor instance', () => {
    const testMonitor = createProcessHealthMonitor();
    expect(testMonitor).toBeInstanceOf(ProcessHealthMonitor);
    testMonitor.stop();
  });
